...
var updateBombsRemaining = function(value) {
    ...
    document.getElementById('bombs').textContent=format(value,2);
}
    ...
    for (i = row - 1; i <= row + 1; i++) {
        for (j = col - 1; j <= col + 1; j++) {
            ...
            if(i>=0&&i<GAME.rows&&j>=0&&j<GAME.cols&&i!=row&&j!=col&&GAME.Cells[i][j].classList.contains("closed"))
                GAME.Cells[i][j].classList.add('indicate');
        }
    }
}
...
var resetNeighbors = function (row, col) {
    ...
    for (i = row - 1; i <= row + 1; i++) {
        for (j = col - 1; j <= col + 1; j++) {
            ...
            // https://www.w3schools.com/jsref/prop_element_classlist.asp
            if(i>=0&&i<GAME.rows&&j>=0&&j<GAME.cols&&i!=row&&j!=col&&GAME.Cells[i][j].classList.contains("closed"))
                GAME.Cells[i][j].classList.remove('indicate');
        }
    }
}
...
var handleMouseEvents = function (mouseButton, row, col, marked) {
    ...
    //       call gameWon() method
    if(GAME.openCellCount==(GAME.rows*GAME.cols-GAME.bombs)||(GAME.flaggedCells+GAME.openCellCount)==(GAME.rows*GAME.cols))
        gameWon();
}
...
var updateNeighborCounts = function (row, col) {
    ...
    for(i=row-1;i<=row+1;i++){
        for(j=col-1; j<=col+1;j++){
            if((i>=0&&i<GAME.rows && j>=0 && j<GAME.cols)&&GAME.layout[i][j]!=-1)
                GAME.layout[i][j]++;
        }
    }
...
var setBombs = function (row, col) {
    ...
    for (var count = 0; count != GAME.bombs;) {
        ...
        if(GAME.layout[i][j]!=-1 && (Math.abs(i-row)>1 && Math.abs(j-col)) ){
            GAME.layout[i][j]=-1;
            count++;
...
var getFormattedTime = function(timeInSeconds) {
    ...
    var h = format(Math.floor(sec/3600),2);
    var m = format(Math.floor((sec-(h*3600))/60),2);
    var s = format(sec-(h*3600)-(m*60),2);
    return h+":"+m+":"+s;
}
