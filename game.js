...
var openCell = function (row, col) {
   ...
   // console.log("adding content: "+con);
...
var validCell = function(row, col)
{
...
var endGame = function () {
    ...
    for (var row = 0; row < GAME.rows; row++) {
        for (var col = 0; col < GAME.cols; col++) {
            ...
            {
                //console.log("opening cell at location "+row+", "+col);
                openCell(row,col);
            }
...
var gameWon = function () {
    stopTimer();
    console.log("GAME WON IS CALLED");
...
var handleLeftClick = function (row, col) {
    ...
    if (validMove(row, col)) { // if user clicked on a closed cell
        if (GAME.layout[row][col] == -1) { // if it is a bomb cell
            ...
            document.getElementById("toon").className = "gametoon icon-emo-unhappy";
            //GAME.layout[row][col].classList.add("blast");
            var cell = GAME.Cells[row][col];
...
var indicateNeighbors = function (row, col) {
    ...
    for (i = row - 1; i <= row + 1; i++) {
        for (j = col - 1; j <= col + 1; j++) {
            var cell = GAME.Cells[row][col];
            if((i < GAME.rows && j < GAME.cols) &&(i!=row&&j!=col)&&(cell.classList.contains("closed")))
...
var handleMouseEvents = function (mouseButton, row, col, marked) {
    ...
    {
        gameWon();
       // console.log((GAME.openCellCount==(GAME.rows*GAME.cols)-GAME.bombs)+" "+(GAME.openCellCount+GAME.flaggedCells==(GAME.rows*GAME.cols)));
        console.log("CALLING GAME WON IN HANDLE MOUSE EVENTS");
    }
...
var createBackFaceContent = function (row, col) {
    ...
    if (GAME.layout[row][col] == -1) { // if bomb
        ...
        //console.log("creating bomb content");
        ...
        //console.log("creating number content");
    } else {  //blank
...
var updateNeighborCounts = function (row, col) {
    ...
    for(i = row - 1; i <= row + 1; i++)
    {
        for(j = col - 1; j <= col + 1; j++)
        {
            if(validCell(i, j) && GAME.layout[i][j]!=-1)
            {
                GAME.layout[i][j]++;
            }
        }
...
var notNeighbours = function(row, col, i, j)
{
...
var getFormattedTime = function(timeInSeconds) {
    ...
    return format(h, 2)+":" + format(m, 2) +":"+ format(s, 2);
